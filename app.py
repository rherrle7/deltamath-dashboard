...
st.markdown("""
    ...
    .dm-header {
        background-color: #2c3e50; padding: 15px 30px; color: white;
        margin: -60px -50px 25px -50px; border-bottom: 4px solid #0056b3;
    }

    .mastery-container { display: flex; justify-content: center; margin-bottom: 20px; }
    .mastery-box {
        background-color: #ffffff; border: 1px solid #d1d5db;
        border-top: 4px solid #0056b3; padding: 10px 40px; text-align: center;
    }
    ...
    .summary-box {
        padding: 15px; background-color: #f8fafc; border: 1px solid #e2e8f0;
        border-radius: 8px; text-align: center; min-height: 150px;
    }
    ...
    .integrity-callout {
        color: #b45309; font-weight: 800; font-size: 0.85rem;
        margin-top: 10px; line-height: 1.2; background: #fffbeb; padding: 4px; border-radius: 4px;
    }

    /* Strategy box without the header line */
    .strategy-container {
        background-color: #eff6ff; border-left: 5px solid #3b82f6;
        padding: 20px; border-radius: 4px; margin: 15px 0 25px 0;
    }
    ...
    .card-header {
        font-size: 1rem; font-weight: 700; padding: 10px;
        border-bottom: 1px solid #e2e8f0; background-color: #f8fafc;
        text-align: center; color: #1e293b;
    }
    .student-row { 
        padding: 8px 15px; border-bottom: 1px solid #f1f5f9; 
        font-size: 1rem; font-weight: 500; display: flex; 
...
if uploaded_file is not None:
    ...
    try:
        ...
        # 3. Restored Strategy Box (Title line removed)
        ...
        with c1:
            ...
            for _, r in speed_gamers.iterrows():
                cn = "color-integrity" if show_colors else ""
                st.markdown(f'<div class="student-row"><span class="{cn}">{r["First"]} {r["Last"]}</span><span class="{cn}">{r[score_col]}%</span></div>', unsafe_allow_html=True)
        ...
        with c2:
            ...
            for _, r in struggling.iterrows():
                cn = "color-tier2" if show_colors else ""
                st.markdown(f'<div class="student-row"><span>{r["First"]} {r["Last"]}</span><span class="{cn}">{r[score_col]}%</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with c3:
            ...
            for _, r in mastery_group.iterrows():
                cn = "color-mastery" if show_colors else ""
                st.markdown(f'<div class="student-row"><span>{r["First"]} {r["Last"]}</span><span class="{cn}">{r[score_col]}%</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        def highlight_logic(row):
            styles = [''] * len(row)
            if not show_colors: return styles
            if row[ppm_col] > 3.5:
                color = 'color: #b45309; font-weight: bold;'
                styles[0], styles[1], styles[row.index.get_loc(score_col)], styles[row.index.get_loc(ppm_col)] = color, color, color, color
            elif row[score_col] < mastery_threshold:
                styles[row.index.get_loc(score_col)] = 'color: #b91c1c; font-weight: bold;'
            else:
                styles[row.index.get_loc(score_col)] = 'color: #15803d; font-weight: bold;'
            return styles

        st.dataframe(
            df.style.apply(highlight_logic, axis=1), 
            use_container_width=True, 
            hide_index=True,
            column_config={ppm_col: st.column_config.NumberColumn(format="%.2f")}
        )
